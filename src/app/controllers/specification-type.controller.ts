...
import SpecificationTypeRepository from '../repositories/SpecificationType/specification-type.repository';

class SpecificationTypeController implements BaseController {
  specificationTypeRepository: SpecificationTypeRepository;
  constructor() {
    this.specificationTypeRepository = new SpecificationTypeRepository();
  }
  async index(req: IRequest, res: IResponse): Promise<ISuccessResponse | IErrorResponse> {
    try {
      const specificationTypes = await this.specificationTypeRepository.find();
      return new SuccessResponse(res, specificationTypes);
    } catch (error) {
...
export default SpecificationTypeController;
