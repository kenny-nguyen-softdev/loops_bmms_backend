...
import TaxTypeRepository from '../repositories/TaxType/tax-type.repository';

class TaxTypeController implements BaseController {
  taxTypeRepository: TaxTypeRepository;
  constructor() {
    this.taxTypeRepository = new TaxTypeRepository();
  }
  async index(req: IRequest, res: IResponse): Promise<ISuccessResponse | IErrorResponse> {
    try {
      const taxTypes = await this.taxTypeRepository.find();
      return new SuccessResponse(res, taxTypes);
    } catch (error) {
...
export default TaxTypeController;
