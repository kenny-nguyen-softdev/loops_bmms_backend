...
class PersonGroupTypeController implements BaseController {
  ...
  async index(req: IRequest, res: IResponse): Promise<ISuccessResponse | IErrorResponse> {
    try {
      const personGroupTypes = await this.personGroupTypeRepository.find();
      return new SuccessResponse(res, personGroupTypes);
    } catch (error) {
...
export default PersonGroupTypeController;
